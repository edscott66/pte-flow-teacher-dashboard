...
- Treat the Pearson PTE Official Benchmark as the authoritative diagnostic reference for the controlled calibration response.
- Before describing an error as a lexical substitution, compare the target and student transcripts word by word. A lexical substitution means one lexical item is replaced by a different lexical item.
- If the same words are present but their sequence or syntactic arrangement has changed, classify the issue as word-order/content sequence disruption, not lexical substitution.
- Do not describe one phrase as replacing another when the evidence shows that the words or phrases have simply been transposed.
- Use the actual student transcript as the evidence source and do not infer an error that is not supported by that transcript.
